...
function domain_hemisphere!(scan::BreastScan{<:AbstractFloat, <:Number, <:Integer}, resolution::Real, radius::Real, dtype::Type)
    if radius <= 0.0
        throw(DomainError(radius, "The radius cannot be 0 or negative"))
    end
    ...
    scan.axes = (-dtype(radius):dtype(resolution):dtype(radius), -dtype(radius):dtype(resolution):dtype(radius), dtype(0):dtype(resolution):dtype(radius))
end

function load_scans!(scan::BreastScan{<:AbstractFloat, <:Number}, source1::String, delim::AbstractChar, T::Type)
    scan.signal = readdlm(source1, delim, T, use_mmap=true)
end

function load_scans!(scan::BreastScan{<:AbstractFloat, <:Number}, source1::String, source2::String, delim::AbstractChar, T::Type)
    scan.signal = readdlm(source1, delim, T, use_mmap=true) .- readdlm(source2, delim, T, use_mmap=true)
end

function load_frequencies!(scan::BreastScan{<:AbstractFloat, <:Number}, source1::String, delim::AbstractChar, T::Type)
    scan.frequencies = readdlm(source1, delim, T, use_mmap=true)
end

function load_antennas!(scan::BreastScan{<:AbstractFloat, <:Number}, source1::String, delim::AbstractChar, T::Type)
    scan.antennas = readdlm(source1, delim, T, use_mmap=true)
end

function load_channels!(scan::BreastScan{<:AbstractFloat, <:Number}, source1::String, delim::AbstractChar, T::Type)
    scan.channels = readdlm(source1, delim, T, use_mmap=true)
end